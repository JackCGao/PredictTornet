...
# Use filesystem-backed sharing to avoid exhausting /dev/shm when many workers are active.
try:
    torch.multiprocessing.set_sharing_strategy("file_system")
except RuntimeError as exc:  # pragma: no cover - defensive fallback
    logging.warning("Could not set torch sharing strategy to file_system: %s", exc)
...
def _wrap_loader_for_lightning(loader: DataLoader) -> DataLoader:
    ...
    loader_kwargs = {
        ...
        # Disable persistent workers to release memory between epochs and shrink shm use.
        "persistent_workers": False,
        "collate_fn": _lightning_collate,
    }
    if loader.num_workers and loader.num_workers > 0:
        # Lower prefetching to reduce concurrent memory/shm usage.
...
def _prepare_dataloader_kwargs(config: Dict) -> Dict:
    ...
    # Keep loader lightweight by default to reduce host memory/shared memory pressure.
...
def main(config: Dict):
    ...
    logging.info(
        "Using %s dataloader with %s workers",
        dataloader_name,
        dataloader_kwargs.get("workers"),
    )
